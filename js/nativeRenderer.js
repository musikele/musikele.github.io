...
/***/ (function(module, exports, __webpack_require__) {
  ...
  var domHelper = _interopRequireWildcard(__webpack_require__(3));
  ...
  var _nativeRenderManager = __webpack_require__(2);
  ...
  var _nativeAssetManager = __webpack_require__(5);
  ...
  function newNativeRenderManager(win) {
    ...
    function fireTracker(adId, event) {
      if (adId === '') {
    ...
    var renderNativeAd = function renderNativeAd(nativeTag) {
      ...
      var targetingData = (0, _utils.transformAuctionTargetingData)(nativeTag);
      var nativeAssetManager = (0, _nativeAssetManager.newNativeAssetManager)(window);
  
      if (nativeTag.hasOwnProperty('adId')) {
        var parsedUrl = (0, _utils.parseUrl)(window.pbNativeData.pubUrl);
        ...
        nativeAssetManager.renderNativeAd(nativeTag.adId, fireNativeCallback);
        fireNativeCallback();
  ...
  */
  ...
  function newNativeAssetManager(win) {
    var callback;
    ...
    function replaceAssets(event) {
      ...
      if (data.message === 'assetResponse') {
        ...
        if (data.hasOwnProperty('rendererUrl') && data.rendererUrl || flag && win.pbNativeData.hasOwnProperty('rendererUrl')) {
          if (win.renderAd) {
            var newHtml = win.renderAd && win.renderAd(data.assets) || '';
    ...
    function renderNativeAd(adId, cb) {
      win.addEventListener('message', receivedHtml, false);
      var message = {
        message: 'Prebid Native',
        action: 'htmlRequest',
        adId: adId
    ...
    function receivedHtml(event) {
      ...
      try {
        ...
        if (errorCountEscapeHatch++ > 10) {
           ...
           * get rid of this listener because other messages won't stop coming
           */
          win.removeEventListener('message', receivedHtml);
        }
  
        return;
      }
  
      if (data.message !== 'htmlResponse') {
        return;
      }
  
      win.document.body.innerHTML = win.document.body.innerHTML + data.html;
      callback && callback();
      win.removeEventListener('message', receivedHtml);
      requestHeightResize(data.adId, document.body.clientHeight || document.body.offsetHeight);
    }
